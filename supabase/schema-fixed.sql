-- ============================================================================
-- DRD Digital - Production-Grade Supabase Schema (FIXED)
...
CREATE INDEX IF NOT EXISTS idx_leads_utm ON leads(utm_source, utm_campaign) WHERE utm_source IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_leads_spam ON leads(spam_score) WHERE spam_score > 50;
...
DROP TRIGGER IF EXISTS inquiries_audit ON inquiries;
...
DROP TRIGGER IF EXISTS increment_leads_version ON leads;
CREATE TRIGGER increment_leads_version BEFORE UPDATE ON leads
  FOR EACH ROW EXECUTE FUNCTION increment_version();

-- ----------------------------------------------------------------------------
-- 9. RATE LIMITING FUNCTIONS (FIXED)
-- ----------------------------------------------------------------------------
...
BEGIN
  -- Check if IP is blocked (FIXED: use SELECT INTO properly)
  SELECT expires_at INTO v_blocked_until
  FROM ip_blocks 
  ...
  DO UPDATE SET 
    ...
    updated_at = now()
  WHERE rate_limits.blocked_until IS NULL
  RETURNING request_count INTO v_count;
  
  -- If v_count is null, it means the record is blocked
  IF v_count IS NULL THEN
    ...
    IF v_count IS NULL THEN
      v_count := p_max_requests + 1; -- Force block
    END IF;
  END IF;
...
-- Inquiries policies
DROP POLICY IF EXISTS "Service role can insert inquiries" ON inquiries;
...
DROP POLICY IF EXISTS "Only admins can manage rate limits" ON rate_limits;
CREATE POLICY "Only admins can manage rate limits" ON rate_limits
  FOR ALL TO authenticated USING (
    EXISTS (SELECT 1 FROM auth.users WHERE id = auth.uid() AND raw_user_meta_data->>'role' = 'admin')
  );

DROP POLICY IF EXISTS "Only admins can manage IP blocks" ON ip_blocks;
CREATE POLICY "Only admins can manage IP blocks" ON ip_blocks
  FOR ALL TO authenticated USING (
    EXISTS (SELECT 1 FROM auth.users WHERE id = auth.uid() AND raw_user_meta_data->>'role' = 'admin')
  );

DROP POLICY IF EXISTS "Only admins can manage email blacklist" ON email_blacklist;
...
-- Lead dashboard view
DROP VIEW IF EXISTS lead_dashboard;
